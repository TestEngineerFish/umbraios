...
struct MoneyCatDTO: Codable, Identifiable, Hashable {
    ...
    /// 图标**语义名**（批次 004：PC 新增分类弹窗挑的那个，gift / pet / …）。
    /// '' 或缺失 = 没挑过，按 slug 兜底 —— 内置分类走的就是这条路。可选给老服务端留余量。
    let icon: String?
...
/// 分类图标的契约（批次 004 起）：服务端存**语义名**（gift / pet / …，用户在
/// PC 新增分类弹窗挑的那一个），两端各自把语义名映射到自己的路径资源 ——
/// 渲染时**存储的语义名优先**，没存（内置分类、老数据）按 slug 兜底。
/// 内置分类的形状以 doc 里 ClaudeDesign 导出的 umbra-icons.json category 组
/// 为正本（该文件自己声明「两端同名图标以本文件为准」，稿内 demo 的 CATS 表
/// 偶尔滞后）；bonus / parttime / invest 三个 json 没收录，沿用稿值。
/// PC 端 moneyKit.ts 的 CAT_ICON / PICK_ICONS 是同一批原始路径，改这边必须同步改那边。
///
...
enum MoneyCatArt {
    /// 可挑的八个图标（批次 004 新增分类 / 批次 006「换图标」共用），顺序照稿。
    /// k 是落库的语义名（只有它进契约），label 是换图标吐司里的中文名，
    /// d 是 iOS 自己的 path 资源（PC 有同名一份自己的）。
    static let pickList: [(k: String, label: String, d: String)] = [
        (k: "gift", label: "礼物", d: "M20,12L20,21L4,21L4,12M2.5,7.5L21.5,7.5L21.5,12L2.5,12ZM12,21.5L12,7.5M12,7.5L7.8,7.5C7.27,7.5 6.76,7.33 6.33,7.02C5.9,6.71 5.59,6.27 5.42,5.77C5.26,5.27 5.26,4.73 5.42,4.23C5.59,3.73 5.9,3.29 6.33,2.98C6.76,2.67 7.27,2.5 7.8,2.5C11,2.5 12,7.5 12,7.5M12,7.5L16.2,7.5C16.73,7.5 17.24,7.33 17.67,7.02C18.1,6.71 18.41,6.27 18.58,5.77C18.74,5.27 18.74,4.73 18.58,4.23C18.41,3.73 18.1,3.29 17.67,2.98C17.24,2.67 16.73,2.5 16.2,2.5C13,2.5 12,7.5 12,7.5"),
        (k: "pet", label: "宠物", d: "M4.6,13.4C4.61,13.8 4.75,14.18 4.99,14.5C5.23,14.82 5.56,15.05 5.94,15.17C6.32,15.28 6.73,15.28 7.11,15.15C7.48,15.02 7.81,14.78 8.04,14.46C8.27,14.14 8.4,13.75 8.4,13.35C8.4,12.95 8.27,12.56 8.04,12.24C7.81,11.92 7.48,11.68 7.11,11.55C6.73,11.42 6.32,11.42 5.94,11.53C5.56,11.65 5.23,11.88 4.99,12.2C4.75,12.52 4.61,12.9 4.6,13.3M9.2,8.4C9.21,8.8 9.35,9.18 9.59,9.5C9.83,9.82 10.16,10.05 10.54,10.17C10.92,10.28 11.33,10.28 11.71,10.15C12.08,10.02 12.41,9.78 12.64,9.46C12.87,9.14 13,8.75 13,8.35C13,7.95 12.87,7.56 12.64,7.24C12.41,6.92 12.08,6.68 11.71,6.55C11.33,6.42 10.92,6.42 10.54,6.53C10.16,6.65 9.83,6.88 9.59,7.2C9.35,7.52 9.21,7.9 9.2,8.3M14.8,8.4C14.81,8.8 14.95,9.18 15.19,9.5C15.43,9.82 15.76,10.05 16.14,10.17C16.52,10.28 16.93,10.28 17.31,10.15C17.68,10.02 18.01,9.78 18.24,9.46C18.47,9.14 18.6,8.75 18.6,8.35C18.6,7.95 18.47,7.56 18.24,7.24C18.01,6.92 17.68,6.68 17.31,6.55C16.93,6.42 16.52,6.42 16.14,6.53C15.76,6.65 15.43,6.88 15.19,7.2C14.95,7.52 14.81,7.9 14.8,8.3M19.4,13.4C19.41,13.8 19.55,14.18 19.79,14.5C20.03,14.82 20.36,15.05 20.74,15.17C21.12,15.28 21.53,15.28 21.91,15.15C22.28,15.02 22.61,14.78 22.84,14.46C23.07,14.14 23.2,13.75 23.2,13.35C23.2,12.95 23.07,12.56 22.84,12.24C22.61,11.92 22.28,11.68 21.91,11.55C21.53,11.42 21.12,11.42 20.74,11.53C20.36,11.65 20.03,11.88 19.79,12.2C19.55,12.52 19.41,12.9 19.4,13.3M12,21C9,21 7,19.2 7,17C7,14.9 8.9,13 12,13C15.1,13 17,14.9 17,17C17,19.2 15,21 12,21"),
        (k: "health", label: "健康", d: "M12,20.5C12,20.5 3.6,15 3.6,9.3C3.62,8.33 3.94,7.39 4.51,6.61C5.09,5.83 5.89,5.24 6.81,4.94C7.73,4.63 8.73,4.62 9.65,4.9C10.58,5.18 11.4,5.74 12,6.5C12.6,5.74 13.42,5.18 14.35,4.9C15.27,4.62 16.27,4.63 17.19,4.94C18.11,5.24 18.91,5.83 19.49,6.61C20.06,7.39 20.38,8.33 20.4,9.3C20.4,15 12,20.5 12,20.5"),
        (k: "sport", label: "运动", d: "M6.5,6.5L6.5,17.5M3.5,9L3.5,15M17.5,6.5L17.5,17.5M20.5,9L20.5,15M6.5,12L17.5,12"),
        (k: "coffee", label: "咖啡", d: "M17,8.5L18,8.5C18.8,8.5 19.58,8.75 20.23,9.23C20.88,9.7 21.37,10.36 21.61,11.13C21.86,11.89 21.86,12.71 21.61,13.47C21.37,14.24 20.88,14.9 20.23,15.37C19.58,15.85 18.8,16.1 18,16.1L17,16.1M3.5,8.5L17,8.5L17,17.1C17,17.77 16.82,18.42 16.49,19C16.16,19.58 15.68,20.06 15.1,20.39C14.52,20.72 13.87,20.9 13.2,20.9L7.3,20.9C6.63,20.9 5.98,20.72 5.4,20.39C4.82,20.06 4.34,19.58 4.01,19C3.68,18.42 3.5,17.77 3.5,17.1ZM6.5,2.5L6.5,4.9M10.2,2.5L10.2,4.9M13.9,2.5L13.9,4.9"),
        (k: "book", label: "书", d: "M4,19.5C4,19.06 4.12,18.63 4.33,18.25C4.55,17.87 4.87,17.55 5.25,17.33C5.63,17.12 6.06,17 6.5,17L20,17M6.5,2.5L20,2.5L20,21.5L6.5,21.5C6.06,21.5 5.63,21.38 5.25,21.17C4.87,20.95 4.55,20.63 4.33,20.25C4.12,19.87 4,19.44 4,19L4,5C4,4.56 4.12,4.13 4.33,3.75C4.55,3.37 4.87,3.05 5.25,2.83C5.63,2.62 6.06,2.5 6.5,2.5"),
        (k: "tag", label: "标签", d: "M20.6,13.4L13.4,20.6C13.15,20.84 12.85,21.02 12.51,21.11C12.18,21.19 11.82,21.19 11.49,21.11C11.15,21.02 10.85,20.84 10.6,20.6L3.4,13.4C3.21,13.22 3.06,13 2.96,12.76C2.86,12.52 2.8,12.26 2.8,12L2.8,4.5C2.8,4.32 2.85,4.15 2.93,4C3.02,3.85 3.15,3.72 3.3,3.63C3.45,3.55 3.62,3.5 3.8,3.5L11,3.5C11.26,3.5 11.52,3.56 11.76,3.66C12,3.76 12.22,3.91 12.4,4.1L20.6,12.3C20.66,12.48 20.7,12.66 20.7,12.85C20.7,13.04 20.66,13.22 20.6,13.4M7.5,7.5L7.51,7.5"),
        (k: "house", label: "房子", d: "M3,10.5L12,3.2L21,10.5M5.3,9.4L5.3,21L18.7,21L18.7,9.4M10,21L10,15L14,15L14,21"),
    ]
    /// 语义名 → path 的查表（渲染热路径用字典，别每次线性扫 pickList）。
    private static let picks: [String: String] =
        Dictionary(uniqueKeysWithValues: pickList.map { ($0.k, $0.d) })
    ...
    /// 图标查找：**存储的语义名优先**（用户挑的要跨端一致），slug 兜底（内置分类），
    /// 都查不到给通用标签形状 —— 历史流水指向停用/未知分类时那行不能消失或崩掉。
    /// stored 缺省 nil 是给拿不到分类对象的老调用方留的（行为与从前一致）。
    static func icon(_ slug: String, stored: String? = nil) -> String {
        if let s = stored, !s.isEmpty, let d = picks[s] { return d }
        return icons[slug] ?? "M3.5,12L12,3.5L20.5,3.5L20.5,12L12,20.5ZM16,8L16.01,8"
    }
