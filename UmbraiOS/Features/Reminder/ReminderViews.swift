    ...
    private func row(_ r: UmbraReminder) -> some View {
        ...
        return Button {
            ...
            HStack(alignment: .top, spacing: UmbraMetric.sp4) {
                ...
                .buttonStyle(.plain)
                // ⚠️ 热区就是视觉的 24×24，**两个轴都不撑**。这是 `minTapTarget.nestedTapTarget`
                // （批次 016）的直接后果，原话：
                //
                //   「凡是『一颗可点的东西嵌在另一颗可点的东西里面』，内层的热区要按
                //    **外层的净空**来定，不能按 44 来定。」
                //
                // 而这一行**从头到尾都是外层 Button 的热区**（整行可点 → 进详情），
                // 净空是零：往上下撑 10pt 借的是行自己的内衬，那块同样归外层；
                // 往左右撑借的是文字列。三个方向都是「向可点的东西借」，规矩明令不许。
                //
                // 演进史：015 只按轴向定规矩（横向不许、纵向可以），所以上一批收了横向、
                // 留了纵向 ±10；016 把判据换成了「溢出方向上有没有可点的东西」，
                // 纵向在这里同样不合格，于是一起收掉。
                //
                // 代价说明白：勾选圈现在 24×24，两个轴都不到 44。**这是我们已经回执给设计侧
                // 的一个悬案** —— 真要给它 44，得先让这一行不再整行可点（比如导航改成只有
                // chevron 那一小块），那是 IA 改动，代码侧定不了。在那之前，
                // 「热区小」好过「按下去改了数据」。
